...
Loader.prototype = {
  ...
  initUserConfig: function (jspath, csspath, done) {
    var alljs = []
      ...
      , types = this.types
    this.allExtensions(function (type, id, plugin, next) {
      var name = types.userConfig || 'user'
        , config = name + 'Config'
      if (!plugin[config]) return next();
      if (!configs[type]) configs[type] = {}
      if (plugin[config] === true) {
      ...
      plugin[config] = _.extend({
        controller: type[0].toUpperCase() + type.slice(1) + 'Controller',
        /* Defaults. If they don't exist, however, no error is thrown.
        template: 'config/account.html',
        script: 'config/account.js',
        style: 'config/account.less',
        */
        icon: plugin.icon,
        title: plugin.title,
        id: id
      ...
      configs[type][id] = plugin[config]
      utils.getConfig(id, plugin.dir, plugin[config], name, function (err, html, css, js) {
        if (err) return next(err)
        plugin[config].html = html
        alljs.push(js)
        allcss.push(css)
