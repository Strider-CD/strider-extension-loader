...
function getConfig(id, basedir, config, done) {
  if (!config.template) {
    throw new Error("Missing template field for plugin " + id)
  }
  var template = path.join(basedir, config.template)
    , script = path.join(basedir, config.script)
    , style = path.join(basedir, config.style)

  async.parallel({
    html: function (next) {
      var failhard = !!config.template
      fs.readFile(path.join(basedir, config.template || 'config/config.html'), 'utf8', function (err, data) {
        next(failhard && err, data)
      })
    },
    js: function (next) {
      var failhard = !!config.script
      fs.readFile(path.join(basedir, config.script || 'config/config.js'), 'utf8', function (err, text) {
        if (!failhard && err) return next(null, '')
        next(err, text + '\n//# sourceUrl=' + id + '/' + path.join(basedir, config.script || 'config/config.js') + '\n')
      })
    },
    css: function (next) {
      var failhard = !!config.style
      fs.readFile(path.join(basedir, config.style || 'config.less'), 'utf8', function (err, text) {
        if (!failhard && err) return next(null, '')
